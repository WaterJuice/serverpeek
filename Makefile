...
dev: go-build
	uv --version 2>/dev/null && true || pip3 install uv
	uv sync
	@ln -sf $$(pwd)/dist/serverpeek-$$(go env GOOS)-$$(go env GOARCH) .venv/bin/serverpeek
	@echo "serverpeek linked into .venv/bin/"

# Run with arguments (e.g. make run ARGS="--port 3000")
.PHONY: run
run: go-build
	@dist/serverpeek-$$(go env GOOS)-$$(go env GOARCH) $(ARGS)
